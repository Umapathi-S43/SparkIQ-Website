...
const FaqItem = ({ num, question, answer, isOpen, toggleAccordion }) => {
  return (
    <div className="px-4 md:px-20">
      <button
        className="w-full flex justify-between items-center py-2 px-4 border border-transparent md:border-0"
      ...
      <div className="flex items-center justify-center">
        <div
          className={`mt-2 mr-2 transition-all duration-300 text-left text-[15px] w-full md:w-1/2 px-2 ${
            isOpen ? "max-h-96 opacity-100 text-justify" : "max-h-0 opacity-0 overflow-hidden"
          }`}
        >
          {answer.split("\n").map((line, index) => (
            <p key={index}>{line}</p>
