...
import { FaChevronRight, FaChevronDown, FaRegLightbulb, FaCheck, FaPlus } from 'react-icons/fa';
...
const BrandSetup = () => {
  ...
  const getDisplayText = (name, description) => {
    ...
    const truncatedDescription = description.split(' ').reduce((acc, word) => {
      if ((acc + word.charAt(0).toUpperCase() + word.slice(1)).length <= availableLength) {
        ...
        <div className="flex flex-col lg:flex-row p-8 w-full mb-2 overflow-y-auto hide-scrollbar" style={{ maxHeight: '64vh' }}>
            ...
            <div className="relative w-60 h-60 sm:w-80 sm:h-80 md:w-96 md:h-96 bg-white rounded-3xl flex items-center justify-center">
              <div className="absolute w-48 h-48 sm:w-64 sm:h-64 md:w-[21rem] md:h-[20rem] bg-[#859398] rounded-3xl flex items-center justify-center">
                <div className="absolute w-36 h-28 sm:w-48 sm:h-40 bg-[rgba(255,255,255,0.24)] rounded-2xl flex items-center justify-center">
                  <img src={brandLogo || gallery} alt="Brand" className="w-32 h-24 sm:w-36 sm:h-28 object-cover rounded-xl" />
                </div>
              </div>
              <div className="absolute top-0 w-full flex flex-col items-center justify-center p-4 text-white">
                <h2 className="text-md sm:text-xl md:text-2xl font-bold capitalize md:pt-8 sm:pt-1">{brandName}</h2>
                <p className="text-xs sm:text-base md:text-lg text-center">{truncateText(brandDescription, 35)}</p>
              </div>
              <img src={sound} alt="Microphone" className="absolute top-0 right-2 w-6 h-6 sm:w-8 sm:h-8" />
            </div>
          </div>
          <div className="flex-grow pr-1">
            <div
              ...
              <div className={`flex items-center justify-between ${expandedSection === 1 ? 'bg-[#F6F8FE]' : ''} p-4 rounded-t-2xl`}>
                <div className="flex items-center">
                  ...
                  <p className="ml-3 lg:text-nowrap">Write Brand Name & Description</p>
                </div>
                {completedSections[1] && (
            ...
            >
              ...
              <div className={`flex items-center justify-between ${expandedSection === 2 ? 'bg-[#F6F8FE]' : ''} p-4 rounded-t-2xl`}>
                <div className="flex items-center">
                  ...
                  <p className="ml-3 flex items-center justify-center">Select Brand Logo</p>
                </div>
                {completedSections[2] && brandLogo && (
                  <div className="flex items-center ml-auto bg-white rounded-lg p-1">
                    <img src={brandLogo} alt="Brand Logo" className="w-12 h-7 object-cover rounded-md" />
                  </div>
                )}
                <div className="ml-4">
              ...
              {expandedSection === 2 && (
                <div className="p-4">
                  <p className='text-sm'>Upload your logo here. A dark-colored logo with a transparent background is recommended.</p>
                    ...
                    <div className='bg-white rounded-xl m-1 p-2 shadow-lg'>
                      <div className="border-dashed border-2 border-gray-400 bg-white rounded-lg p-1 m-1 text-center cursor-pointer hover:border-gray-600 relative">
                        ...
                        <label htmlFor="file-upload" className="flex flex-col items-center justify-center h-full cursor-pointer">
                          ...
                          <span className="text-gray-500 text-nowrap">Upload a logo here</span>
                        </label>
                      </div>
                    </div>
                  </div>
            ...
            <div
              ...
              className={`relative items-center border border-[#fcfcfc] p-0 rounded-2xl cursor-pointer ${expandedSection === 3 ? 'bg-[rgba(252,252,252,0.25)]' : ''}`}
            >
              {completedSections[3] && (
                <div className="absolute -top-3 -right-6 flex items-center bg-[#A7F3D0] text-[#059669] px-2 py-1 rounded-xl">
                  <div className='text-xs'>Completed</div>
              ...
              <div className={`flex items-center justify-between ${expandedSection === 3 ? 'bg-[#F6F8FE]' : ''} p-4 rounded-t-2xl`}>
                <div className="flex items-center">
                  <div className="bg-[rgba(0,39,153,0.15)] rounded-full p-2">
                    <FaRegLightbulb className="text-[#374151] text-xl" />
                  </div>
                ...
                <div className="p-2" onClick={handlePickerClick}>
                  <div className="flex flex-wrap items-center gap-4 p-2 rounded-xl">
                    <div className="flex items-center bg-white p-2 rounded-xl">
                      <label className="text-sm pl-4 font-semibold lg:pr-14 pr-10 text-nowrap">Brand Color 1</label>
                      <span
                    ...
                    <div className="flex items-center bg-white p-2 rounded-xl">
                      <label className="text-sm pl-4 font-semibold lg:pr-14 pr-10 text-nowrap">Brand Color 2</label>
                      <span
                        className="h-8 p-3 rounded-lg flex items-center justify-center text-white font-normal text-sm cursor-pointer"
                        style={{ background: brandColor2 }}
                        onClick={() => {
                          setCustomColor(brandColor2);
                          setColorPickerTarget(null);
                          setColorPickerOpen(true);
                        }}
                      >
                    ...
                    {additionalColors.map((color, index) => (
                      <div key={index} className="flex items-center bg-white p-2 rounded-xl">
                        <label className="text-sm pl-3 font-semibold lg:pr-9 pr-4 text-nowrap">Additional Color {index + 1}</label>
                        <span
                          className="h-8 p-3 rounded-lg flex items-center justify-center text-white font-normal text-sm cursor-pointer"
            ...
            <div className="flex justify-start mt-4">
              <button className="custom-button p-2 pl-6 ml-2 pr-6 text-white rounded-lg" onClick={handleCreateBrand}>
                Create Brand
              </button>
            </div>
          </div>
