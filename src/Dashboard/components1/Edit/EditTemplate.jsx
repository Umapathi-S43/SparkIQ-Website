...
const initialElements = [
  ...
  },
  {
    ...
    style: { fontSize: '18px', fontFamily: 'Arial', whiteSpace: 'normal', wordWrap: 'break-word' },
  },
  {
    id: 3,
    type: 'text',
    content: 'Surveyors and engineers to measure the change in position of a target that is assumed to be moving.',
    defaultPosition: { x: 20, y: 275 },
    defaultSize: { width: 360, height: 50 },
    style: { fontSize: '14px', fontFamily: 'Arial', whiteSpace: 'normal', wordWrap: 'break-word' },
  },
  {
    id: 4,
    type: 'button',
    ...
    style: { fontSize: '14px', fontFamily: 'Arial', backgroundColor: 'blue', color: 'white', textAlign: 'center', lineHeight: '40px', borderRadius: '4px' },
  },
  {
    id: 5,
    type: 'text',
...
export default function EditTemplate() {
  ...
  const handleResize = (e, direction, ref, delta, id) => {
    ...
    const updatedElements = elements.map(el => (el.id === id ? { ...el, defaultSize: newSize } : el));
    setElements(updatedElements);
    saveHistory(updatedElements);
  };

  const handleDragStop = (e, d, id) => {
    const updatedElements = elements.map(el => (el.id === id ? { ...el, defaultPosition: { x: d.x, y: d.y } } : el));
    setElements(updatedElements);
    saveHistory(updatedElements);
  };
  ...
  const onItemSelected = (item, tab) => {
    if (tab === 'Templates') {
      ...
      setElementsAndSaveHistory(initialElements.map((el, idx) => ({ ...el, defaultPosition: { ...el.defaultPosition, x: idx * 10, y: idx * 10 } })));
    }
  };
  ...
  const onDrop = (e) => {
    ...
    setElementsAndSaveHistory([...elements, { ...item, id: elements.length + 1 }]);
  };
  ...
  const handleInputChange = (e, id) => {
    const updatedElements = elements.map(el => (el.id === id ? { ...el, content: e.target.value } : el));
    setElements(updatedElements);
    saveHistory(updatedElements);
  };
  ...
  const titleSuggestions = ['Surveying prisms - Geomatics', 'Geomatics-prims', 'Surveying prism', 'Optical survey prism', 'Mini prism'];
  ...
  const handleSuggestionClick = (field, suggestion) => {
    const updatedElements = elements.map(el => {
      if (field === 'title' && el.id === 2) return { ...el, content: suggestion };
      if (field === 'description' && el.id === 3) return { ...el, content: suggestion };
  ...
  return (
    <div className="min-h-screen p-2 bg-gradient-to-b from-[#B3D4E5] to-[#D9E9F2] flex flex-col items-center justify-center">
      <div className="border-2 border-white rounded-[32px] w-full" style={{ height: 'calc(100vh - 1rem)' }}>
        ...
        <div className="flex">
          <div className="p-4 w-1/12">
            <Sidebar setActiveComponent={setActiveComponent} setShowAdCreatives={setShowAdCreatives} />
          </div>
          <div className="flex-row relative w-full m-4 ml-0">
            <div className="flex items-center justify-between p-4 w-full bg-[#FCFCFC40] shadow-lg rounded-md mt-1" style={{ height: '8vh' }}>
              <div className="flex">
                <button onClick={handleUndo} className="rounded mr-2 relative group">
                ...
                <button onClick={handleRedo} className="rounded mr-2 relative group">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-4">
                ...
                {activeComponent === 'Text' && <TextFormatToolbar onClose={() => setActiveComponent('')} />}
              </div>
              <div className="flex">
            ...
            <div
              ...
              <div className="w-1/2 h-[450px] flex flex-col items-start p-4 mr-12 mt-0 pt-0">
                {showAdCreatives ? (
                  <AdCreatives onItemSelected={onItemSelected} onClose={() => setShowAdCreatives(false)} />
                ) : (
                  ...
                  <div className="flex items-center mt-4">
                    ...
                    <div className="relative flex-1">
                      ...
                      <div className="flex flex-row">
                        <input
                        type="text"
                        value={elements.find(el => el.id === 2)?.content}
                        onChange={(e) => handleInputChange(e, 2)}
                        className="input lg:w-[387px] w-3/4 py-2 px-4 rounded-lg focus:ring-2 focus-within:ring-blue-400 focus:outline-none"
                      />
                      <button className="flex items-center shadow-lg bg-[#FCFCFC60] border-2 px-3 rounded-md py-1 ml-2" onClick={() => toggleSuggestions('description')}>
                        <img
                          src="icon7.svg"
                          className="w-7 h-7 cursor-pointer -mt-1"
                        />
                        <p className="text-xs ml-1 text-nowrap">AI Assist</p>
                      </button>
                      </div>
                  ...
                  <div className="flex flex-row">
                    <textarea
                        value={elements.find(el => el.id === 3)?.content}
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="relative flex-1">
                      ...
                      <input
                        type="text"
                        value={elements.find(el => el.id === 4)?.content}
                        ...
                        <div className="absolute z-10 w-full mt-1 bg-white shadow-lg rounded-md">
                          {suggestions.cta.map((suggestion, index) => (
                            <div
                              key={index}
                              className="px-4 py-2 cursor-pointer hover:bg-gray-200"
                              onClick={() => handleSuggestionClick('cta', suggestion)}
                            >
                              {suggestion}
                            </div>
                          ))}
                  ...
                  <div className="flex items-center">
                    ...
                    <input
                      type="text"
                      value={elements.find(el => el.id === 5)?.content}
                  ...
                  <div className="flex items-center">
                    ...
                    <input
                      type="text"
                      value={elements.find(el => el.id === 6)?.content}
                ...
                >
                  {elements.map(el => (
                    <Rnd
                      key={el.id}
                      default={{
                        x: el.defaultPosition.x,
                        y: el.defaultPosition.y,
                        width: el.defaultSize.width,
                        height: el.defaultSize.height,
                      }}
                      bounds="parent"
                      onResize={(e, direction, ref, delta) => handleResize(e, direction, ref, delta, el.id)}
                      ...
                      style={{
                        border: activeElementId === el.id ? '2px dashed #000' : 'none',
                        boxSizing: 'border-box',
                      }}
                    >
                      {el.type === 'image' ? (
                        <div className="flex flex-col items-center w-full h-full">
                          <img src={el.src} alt="Ad Image" className="object-contain w-full h-full" />
                        </div>
                      ) : el.type === 'text' ? (
                        <div
                        ...
                        <div style={{ ...el.style, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          {el.content}
                        </div>
                      ) : null}
                    </Rnd>
...
const Sidebar = ({ setActiveComponent, setShowAdCreatives }) => {
  return (
    <div className="flex flex-col items-center p-3 rounded-[12px] bg-[#FCFCFC40] shadow-lg" style={{ height: 'calc(100vh - 10rem)' }}>
