...
export default function AdCreatives({ onItemSelected, onClose }) {
  ...
  const getCardClass = (tab, idx) => {
    if (tab === "Gradient Ads")
      ...
      } p-4 rounded-lg shadow-md w-60`;
    if (tab === "AI Backgrounds")
      return `bg-white p-4 rounded-lg shadow-md w-60 ${
        aiBackgrounds[idx % aiBackgrounds.length]
      }`;
    if (tab === "Video Ads")
      return `bg-gray-100 p-4 rounded-lg shadow-md w-60 animate-slide-${
        idx % 6
      }`;
  ...
  return (
    <div className="flex flex-col items-center p-3 border border-[#FCFCFC] rounded-[12px] shadow-lg">
      ...
      <div className="flex justify-between w-full mb-3">
        {["Templates", "Gradient Ads", "AI Backgrounds", "Video Ads"].map(
          (tab) => (
            <button
              key={tab}
              className={`px-4 py-1 ${
                selectedTab === tab
                  ? "border-b-2 border-[#082A66] text-[#082A66]"
                  : ""
              }`}
      ...
      <div
        className="grid grid-cols-2 gap-4 mb-4 pr-4 pb-2 overflow-auto"
      ...
      >
        {[...Array(6)].map((_, idx) => (
          ...
          >
            ...
            />
            <h3 className="text-sm font-bold justify-items-start">
              Surveying prisms - Geomatics
            </h3>
            <p className="text-xs mb-2">
              Surveyors and engineers to measure the change in position of a
              target that is assumed to be moving.
            </p>
