      ...
      <>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredProducts.map((product, index) => (
            ...
            >
              <img
                src={product.imageURL || product.generatedImage}
                alt={product.title}
  ...
  return (
    <div ref={generatedCreativesRef} className="container max-w-7xl mt-4 lg:p-0 flex-grow mx-auto">
      ...
      `}</style>
        ...
        <section
          className={`border border-white bg-[rgba(252,252,252,0.25)] rounded-[24px] ${isThirdSectionOpen ? "p-0 pb-6" : "p-3"
            } flex flex-col gap-2 relative z-10`}
        >
          <div
            className={`flex justify-between items-center bg-[rgba(252,252,252,0.40)] ${isThirdSectionOpen ? "rounded-t-[20px] p-4" : "rounded-[20px] p-2"
              ...
              <div className="flex justify-center">
                <div className="tab-buttons flex justify-center items-center gap-2 px-2 mb-0 w-fit border-4 py-1 rounded-xl shadow-md">
                  {["Brand Color", "Single Color", "Gradient Color", "Templates"].map((tab) => (
                    <button
                      key={tab}
                      className={`px-6 py-2 rounded-lg ${selectedTab === tab
