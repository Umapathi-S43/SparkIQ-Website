...
/*---------------------------------------------------------------------
  CUSTOM "Design" SECTION WITH COLOR PALETTES + TEMPLATES
---------------------------------------------------------------------*/
const CustomSection = {
  name: "custom",
  ...
  Panel: observer(({ store }) => {
    // Track brandId in React state so changes trigger a re-render
    ...
    const fetchColorPalettes = async () => {
      ...
      try {
        ...
        const parsed = raw.map((item) => {
          ...
          return { id: item.id, colors: colorsArray };
        });
    ...
    // Track changes to the palette color pickers
    ...
    const applyPalette = (palette) => {
      ...
      activePage.set({ background: bgColor });

      // Apply to shapes/elements
    ...
    const handlePaletteHover = (palette) => {
      ...
      // Save the current state, apply palette, revert on mouse leave
      store.history.save();
    ...
    const applyTemplate = (template) => {
      ...
      try {
        ...
        // Clear existing pages and load from JSON
        store.deletePages(store.pages.map((p) => p.id));
        store.loadJSON(parsed);

        // Optionally store the loaded template ID somewhere
    ...
    return (
      <div style={{ padding: "4px" }}>
        {/* Tab Switcher: Palettes vs Templates */}
        ...
        {activeTab === "palettes" && (
          <div style={{ width: "100%", marginTop: "14px" }}>
            ...
            {!loadingPalettes && colorPalettes.length === 0 && <p>No color palettes found.</p>}
        ...
        {activeTab === "templates" && (
          <div style={{ marginTop: "10px" }}>
            ...
            {!loadingTemplates && templates.length === 0 && <p>No templates available.</p>}
            <div
              style={{
                display: "grid",
...
const PolotnoEditor = () => {
  ...
  Panel: observer(({ store }) => {
    ...
    // Handle file uploads => create brand element => refetch
        ...
        >
          {brandElements.map((element, index) => (
            <div
              ...
              onClick={() => {
                // Add the selected image to the current page
...
const PolotnoEditor = () => {
  ...
  // Grab template data from route state
  const template = state?.templateData || {};
  console.log("Incoming template data:", template);

  // Keep brandId in React state; fallback to what's in template or localStorage
  const [brandId, setBrandId] = useState(() => {
    return template?.brandId || localStorage.getItem("brandId") || "";
  });

  useEffect(() => {
    // Whenever brandId changes, update localStorage
    if (brandId) {
      localStorage.setItem("brandId", brandId);
    }
  }, [brandId]);
  ...
  // -------------------------------------
  // SAVE Template as JSON (with thumbnail)
  // -------------------------------------
  ...
  const saveAsJSON = async (isUpdate = false) => {
    try {
      ...
      const json = store.toJSON();
      const template_original = localStorage.getItem("loadedtemplate")
        ? JSON.parse(localStorage.getItem("loadedtemplate"))
        : {};

      const payload = {
        templateId: isUpdate && currentTemplateId ? currentTemplateId : "",
        url: thumbnailURL,
        templateOrientation:
      ...
      // 4) POST (save) template
      const apiResponse = await axios.post(`${baseUrl}/v2/user/templates`, payload, {
        headers: { Authorization: `Bearer ${jwtToken}` },
      });

      // If successfully created a new template, remember its ID
  ...
  // -------------------------------------
  // On mount, load the template data
  // -------------------------------------
  useEffect(() => {
    ...
    if (templateData && Object.keys(templateData).length > 0) {
      console.log("Loading template data into Polotno...");
      store.loadJSON(templateData);
      console.log("Loaded template data into Polotno.");
      if (template.templateId) {
        setCurrentTemplateId(template.templateId);
      }
    } else {
      // If no pages yet, add one
      if (store.pages.length === 0) {
        store.addPage();
      }
    }
  ...
  // -------------------------------------
  // Combine sections for the side panel
  // -------------------------------------
  const sections = [
    CustomSection,            // Our "Design" section (palettes + templates)
    TextSection,
    PhotosSection,
    ElementsSection,
    UploadSectionWithAPI,     // Our "Upload" section with brand elements
    BackgroundSection,
    LayersSection,
    SizeSection,
  ];
  ...
  return (
    ...
    >
      ...
      >
        {/* Left or middle controls could go here... */}

        {/* Right Controls: Theme Toggle, Save, Close */}
        <div
          style={{
            display: "flex",
            gap: "4px",
        ...
        >
          ...
          <button
            className="close"
            onClick={() => {
              // Clear pages/elements so next time is empty
              store.deletePages(store.pages.map((p) => p.id));
              // Then navigate away
      ...
      {/* Polotno Container (Side Panel + Workspace) */}
