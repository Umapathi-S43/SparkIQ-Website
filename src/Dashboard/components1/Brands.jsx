      ...
      <div className="max-w-6xl mx-auto border border-[#fcfcfc] rounded-3xl flex flex-col items-center">
        <div className="w-full bg-[rgba(252,252,252,0.40)] rounded-t-3xl p-4">
          <div className="flex items-center justify-between">
            ...
            <img src={brandImage} alt="Brand Banner" className="w-[180px] h-[90px] mr-20 md:-mb-4 hidden lg:block" />
          </div>
        </div>
          ...
          <div className="w-full flex flex-wrap justify-start gap-4 ml-4 pl-2">
            <div className='flex items-center justify-center bg-[rgba(252,252,252,0.70)] border border-white rounded-3xl p-1 w-60 h-72 mr-3 hover:bg-[rgba(252,252,252,0.15)]'>
              <div
                ...
                <p className="text-sm text-center mt-2">Click here to add an additional brand that you can use to generate assets.</p>
              </div>
            </div>
              ...
              >
                <div className="relative flex flex-col items-center justify-center text-white w-full h-full overflow-hidden">
                  ...
                  <div className="absolute top-8 bg-white p-2 rounded-lg mt-0 mb-8">
                    <img src={brand.logo} alt={brand.name} className="w-16 h-16 object-cover rounded-lg group-hover:mt-0" />
                  </div>
                  <div className="absolute top-30 text-center">
                    <h3 className="text-xl font-bold mb-2 group-hover:text-xl">{brand.name}</h3>
                    <p className="text-md font-thin text-start ml-4 truncate-line-clamp group-hover:font-light">{truncateText(brand.description, 55)}</p>
                  </div>
                  <div className="absolute top-52 text-sm mt-4 flex items-center">
                    <img src={picon} alt="Products Icon" className="w-5 h-5 p-1 mr-2 opacity-80 rounded-md" style={{ background: 'rgba(0, 0, 0, 0.2)' }} />
                    <span className="opacity-80">{brand.productsCreated} Products Created</span>
                  </div>
                </div>
              </div>
            ))}
